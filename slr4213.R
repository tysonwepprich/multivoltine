.tmplib <- lapply(c('base','methods','datasets','utils','grDevices','graphics','stats','rslurm','devtools','msm','parallel','plyr','dplyr','StopoverCode'), 
           library, character.only = TRUE, quietly = TRUE) 
load('dataIN.RData') 
load('slr4213.RData') 
.rslurm_id <- as.numeric(Sys.getenv('SLURM_ARRAY_TASK_ID')) 
.rslurm_istart <- .rslurm_id * 125 + 1 
.rslurm_iend <- min((.rslurm_id + 1) * 125, 
                    nrow(.rslurm_params)) 
...
save(.rslurm_result, file = paste0('slr4213_', .rslurm_id, '.RData'))