.tmplib <- lapply(c('base','methods','datasets','utils','grDevices','graphics','stats','nlme','mgcv','plyr','dplyr','ggplot2','parallel','reshape2','data.table','rslurm','devtools','msm','StopoverCode'), 
           library, character.only = TRUE, quietly = TRUE) 
load('dataIN.RData') 
load('slr1539.RData') 
.rslurm_id <- as.numeric(Sys.getenv('SLURM_ARRAY_TASK_ID')) 
.rslurm_istart <- .rslurm_id * 8 + 1 
.rslurm_iend <- min((.rslurm_id + 1) * 8, 
                    nrow(.rslurm_params)) 
...
save(.rslurm_result, file = paste0('slr1539_', .rslurm_id, '.RData'))