.tmplib <- lapply(c('base','methods','datasets','utils','grDevices','graphics','stats','rslurm','devtools','msm','parallel','plyr','dplyr','StopoverCode','ggplot2'), 
           library, character.only = TRUE, quietly = TRUE) 
load('dataIN.RData') 
load('slr3668.RData') 
.rslurm_id <- as.numeric(Sys.getenv('SLURM_ARRAY_TASK_ID')) 
.rslurm_istart <- .rslurm_id * 14 + 1 
.rslurm_iend <- min((.rslurm_id + 1) * 14, 
                    nrow(.rslurm_params)) 
...
save(.rslurm_result, file = paste0('slr3668_', .rslurm_id, '.RData'))